...
;; 5. Generators (using in-generator to iterate over yielded values)
...
;; =============================================================================
;; 5. Generators: Producing Values on Demand using in-generator
;; =============================================================================
(require racket/generator)
(displayln "\n=== Generators ===")
;;
;; Note: In older examples one might see the use of a function called 
;; `generator-next` to retrieve values one-by-one. In current Racket versions,
;; that identifier is not exported. Instead, we use the sequence interface
;; for generators via `in-generator` to iterate over the yielded values.
;;
...
(for ([val (in-generator simple-gen)])
  (displayln val))
;; Once the generator is exhausted, the loop simply ends.
...
;; Using in-range to limit the iteration since this generator is infinite.
(for ([v (in-generator fib-gen)]
      [i (in-range 10)])
  (displayln v))
